...
#                                                                                      #
# WARNING: This file and all related programs (including any computer programs,        #
# example programs, and all source code) are the unpublished and exclusive property    #
# of Trading Technologies International, Inc. ("TT"), are protected by copyright       #
# law and international treaties, and are for use only by those with the express       #
# written permission from TT.  Unauthorized possession, reproduction, distribution,    #
# use or disclosure of this file and any related program (or document) derived from    #
# it is prohibited by State and Federal law, and by local law outside of the U.S.      #
# and may result in severe civil and criminal penalties. They must be held in strict   #
# confidence.                                                                          #
#                                                                                      #
########################################################################################
...
def build_enums(environment, headers):
    ...
    for enum in prod_data:
        if enum in ('status', 'message'):
            continue
        enums[enum] = {int(info['id']): info['desc'] for info in prod_data[enum]}
